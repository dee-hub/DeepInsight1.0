...
# dashboard title
st.title('DeepInsight v2.0')
st.markdown("Real-Time Presidential Race / Twitter Analysis ")
st.markdown(meta.SUB_HEADER, unsafe_allow_html=True)
# top-level filters
selected = st.selectbox("Select the Candidate", index=0, options=["Peter Obi", "Bola Ahmed Tinubu", "Atiku Abubakar"])
if selected == "Peter Obi":
    selected = "peterobi"
elif selected == "Bola Ahmed Tinubu":
    selected = "bat"
elif selected == "Atiku Abubakar":
    selected = "atiku"
...
# create the pie chart
    ...
    if keyword1 == "peterobi":
        colors = ['#45cc48', '#ed3434']
    elif keyword1 == "bat":
        colors = ['blue', 'yellow']
    elif keyword1 == "atiku":
    ...
    ax1.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors, explode=explode, pctdistance=0.85)
...
def compare_with_other_candidates(keyword1):
    sentimentPO = sentiment_analysis("peterobi")
    sentimentBAT = sentiment_analysis("bat")
    sentimentAtiku = sentiment_analysis("atiku")

# count the number of positive and negative sentiments
    pos_count_po = sentimentPO[sentimentPO['sentiment'] == 'positive']['sentiment'].count()
    neg_count_po = sentimentPO[sentimentPO['sentiment'] == 'negative']['sentiment'].count()

    pos_count_bat = sentimentBAT[sentimentBAT['sentiment'] == 'positive']['sentiment'].count()
    neg_count_bat = sentimentBAT[sentimentBAT['sentiment'] == 'negative']['sentiment'].count()

    pos_count_atiku = sentimentAtiku[sentimentAtiku['sentiment'] == 'positive']['sentiment'].count()
    neg_count_atiku = sentimentAtiku[sentimentAtiku['sentiment'] == 'negative']['sentiment'].count()

# create the pie chart
    labels = ['Positive', 'Negative']
    sizes_po = [pos_count_po, neg_count_po]
    sizes_bat = [pos_count_bat, neg_count_bat]
    sizes_atiku = [pos_count_atiku, neg_count_atiku]
    ...
    fig1, ax1 = plt.subplots()
    if keyword1 == "peterobi":
        ax1.pie(sizes_po, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors, explode=explode, frame=True, radius=5, pctdistance=0.9)
        ax1.pie(sizes_bat, autopct='%1.1f%%', startangle=90, colors=colors2, explode=explode2, radius=3.5, pctdistance=0.8)
        ax1.pie(sizes_atiku, autopct='%1.1f%%', startangle=90, colors=colors3, explode=explode2, radius=2, pctdistance=0.5)
        plt.legend(loc=(0.9, 0.1))
        handles, labels = ax1.get_legend_handles_labels()

        ax1.legend(handles[3:], labels, loc=(0.9, 0.1))
    ...
    #st.pyplot(fig1)
        ...
        ax1.pie(sizes_bat, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors2, explode=explode, frame=True, radius=5, pctdistance=0.9)
        ax1.pie(sizes_po, autopct='%1.1f%%', startangle=90, colors=colors, explode=explode2, radius=3.5, pctdistance=0.8)
        ax1.pie(sizes_atiku, autopct='%1.1f%%', startangle=90, colors=colors3, explode=explode2, radius=2, pctdistance=0.5)
        plt.legend(loc=(0.9, 0.1))
        handles, labels = ax1.get_legend_handles_labels()
    ...
    #st.pyplot(fig1)
        ...
        ax1.pie(sizes_atiku, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors3, explode=explode, frame=True, radius=5, pctdistance=0.9)
        ax1.pie(sizes_po, autopct='%1.1f%%', startangle=90, colors=colors, explode=explode2, radius=3.5, pctdistance=0.8)
        ax1.pie(sizes_bat, autopct='%1.1f%%', startangle=90, colors=colors2, explode=explode2, radius=2, pctdistance=0.5)
        plt.legend(loc=(0.9, 0.1))
        handles, labels = ax1.get_legend_handles_labels()
...
for seconds in range(50000):
    extract_tweets("Peterobi", "obidatti", number_of_tweets_to_retrieve=5)
    extract_tweets("BAT", "TinubuShettima", number_of_tweets_to_retrieve=5)
    extract_tweets("Atiku", "AtikuOkowa", number_of_tweets_to_retrieve=5)
    ...
    with placeholder.container():
        ...
        data1.metric(
            ...
            delta=No_of_tweets - 50,
        )
        
        data2.metric(
            label="Positive Sentiments 🟢",
            value=int(positive_sentiments),
            delta= int(positive_sentiments) - 10,
        )
        
        data3.metric(
            label="Negative Sentiments 🔴",
            value=int(negative_sentiments),
            delta= int(negative_sentiments) - 10,
        )
        ...
        with fig_col2:
            ...
            st.write(compare_with_other_candidates(keyword1=selected))
        ...
        st.dataframe(sentiment_analysis(keyword1=selected)[['Raw tweet', 'sentiment']].tail())
        ...
        st.markdown(meta.DEVELOPER, unsafe_allow_html=True)
        time.sleep(60)
